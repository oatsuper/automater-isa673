...
public class AutoDroidDbAdapter {
	...
	public AutoDroidDbAdapter(Context ctx) {
		this.mCtx = ctx;
	}

	public AutoDroidDbAdapter open() throws SQLException {
...
//			Log.i(TAG, "AddTouch");
	...
	public long getFirstTouchTime(long runId) throws SQLException {
		...
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			long result = mCursor.getLong(mCursor.getColumnIndex(AutoDroidDbAdapter.KEY_CURRENT_TIME));
			mCursor.close();
			return result;
		}
		return -1;
	...
	public boolean isRunning(long runId) throws SQLException {
		...
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			String result = mCursor.getString(mCursor.getColumnIndex(AutoDroidDbAdapter.KEY_STATE));
			mCursor.close();
