					...
					for (int i = 0; i < count; i++) {
						...
						for (int j = 0; j < runCursor.getCount(); j++) {
							command = "sendevent " + Constants.DEVICE_TOUCHPAD + " " 
								+ runCursor.getLong(runCursor.getColumnIndex(AutoDroidDbAdapter.KEY_TYPE)) + " "
								+ runCursor.getLong(runCursor.getColumnIndex(AutoDroidDbAdapter.KEY_CODE)) + " "
								+ runCursor.getLong(runCursor.getColumnIndex(AutoDroidDbAdapter.KEY_VALUE));
							currTime = runCursor.getLong(runCursor.getColumnIndex(AutoDroidDbAdapter.KEY_CURRENT_TIME));
